# 08-Find users with browse permission [GET]
...
| `issueKey` | `string` | `query` | No | The issue key for the issue. Required, unless `projectKey` is specified. |
| `projectKey` | `string` | `query` | No | The project key for the project (case sensitive). Required, unless `issueKey` is specified. |
