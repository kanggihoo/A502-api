# 14-Remove issue security level [DELETE]
...
| `levelId` | `string` | `path` | Yes | The ID of the issue security level to remove. |
| `replaceWith` | `string` | `query` | No | The ID of the issue security level that will replace the currently selected level. |
