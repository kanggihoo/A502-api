...
• 매우 중요함 : Jenkins 연동 핵심 API.
• POST /api/v4/projects/{id}/statuses/{sha}: Jenkins에서 빌드 진행 중(running), 성공(success), 실패(failed) 결과를 GitLab 커밋/MR
화면(녹색 체크/빨간 X)에 보고할 때 사용하는 외부 CI 전용 API.
• Jenkins GitLab Plugin도 내부적으로 이 API 호출함.
