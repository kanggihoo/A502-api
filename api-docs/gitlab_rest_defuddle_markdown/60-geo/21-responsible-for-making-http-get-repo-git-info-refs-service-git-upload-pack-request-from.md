# 21-Responsible for making HTTP GET /repo.git/info/refs?service=git-upload-pack
                  request from secondary gitlab-shell to primary [POST]

`POST /api/v4/geo/proxy_git_ssh/info_refs_upload_pack`
...
```json
{
  "secret_token": string (required), // Secret token to authenticate by gitlab shell
  "data": {
