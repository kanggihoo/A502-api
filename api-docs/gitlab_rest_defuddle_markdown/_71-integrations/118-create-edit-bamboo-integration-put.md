# 118-Create/Edit Bamboo integration [PUT]
...
```json
{
  ...
  "bamboo_url": string (required), // Bamboo root URL (for example, `https://bamboo.example.com`).
  "build_key": string (required), // Bamboo build plan key (for example, `KEY`).
  "username": string (required), // User with API access to the Bamboo server.
