...
```json
{
  ...
  "slug": string,
  "tier": enum("production" | "staging" | "testing" | "development" | "other"), // The tier of the new environment. Allowed values are `production`, `staging`, `testing`, `development`, and `other`
  "cluster_agent_id": integer, // The ID of the Cluster Agent to associate with this environment
  ...
  "auto_stop_setting": enum("always" | "with_action"), // The auto stop setting for the environment. Allowed values are `always` and `with_action`
}
```
### Responses
